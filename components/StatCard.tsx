...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon, gradient, glowColor, subtitle, trend }) => {
  return (
    <motion.div 
      ...
      className="relative bg-white rounded-[2rem] p-6 sm:p-8 border border-slate-100 flex flex-col justify-between overflow-hidden group h-full shadow-sm hover:shadow-md transition-shadow"
    >
      ...
      <div className="flex items-start justify-between relative z-10 mb-6">
        ...
        {trend && (
          <div className="px-3 py-1 rounded-full bg-slate-50 border border-slate-100 flex items-center gap-2">
            ...
            <span className="text-[9px] font-bold text-slate-600 uppercase tracking-wider">{trend}</span>
          </div>
        )}
      </div>
